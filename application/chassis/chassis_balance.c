...
// 一些内部的配置
#define ENABLE_EXHIBITION_MODE false  // 启用展会模式
...
static void BodyMotionObserve(void)
{
    ...
    if (fabs(CHASSIS.ref.speed_vector.vx) < WHEEL_DEADZONE &&
        fabs(CHASSIS.fdb.body.x_dot_obv) < 0.5f) {
...
void ChassisReference(void)
{
    ...
    static float phi = 0;
    if (ps2_btns.button[PS2_L2].now) {
        phi += 0.004f;
    } else if (ps2_btns.button[PS2_R2].now) {
        phi -= 0.004f;
    }
    CHASSIS.ref.leg_state[0].phi = phi;
    CHASSIS.ref.leg_state[1].phi = phi;
...
void ChassisReference(void)
{
    ...
    switch (CHASSIS.mode) {
        ...
        case CHASSIS_FOLLOW_GIMBAL_YAW: {  // 云台跟随模式下，控制量为云台坐标系下的速度，需要进行坐标转换
            ...
            CHASSIS.ref.speed_vector.vx = v_set.vx * cosf(delta_yaw);
            CHASSIS.ref.speed_vector.vy = 0;
            if (v_set.wz > 3.0f || v_set.wz < -3.0f) {  // 小陀螺
                CHASSIS.ref.speed_vector.wz = v_set.wz;
            } else {
                if (GetGimbalInitJudgeReturn()) {
                    CHASSIS.ref.speed_vector.wz = 0;
...
static void LocomotionController(void)
{
    ...
    float L_diff = PID_calc(&CHASSIS.pid.roll_angle, CHASSIS.fdb.body.roll, CHASSIS.ref.body.roll);
